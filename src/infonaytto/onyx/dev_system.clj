...
(defrecord OnyxDevEnv [n-peers]
  ...
  (start [component]
    ...
    (let [onyx-id (java.util.UUID/randomUUID)
          env-config (assoc (-> "env-config.edn" resource slurp read-string) :onyx/tenancy-id onyx-id)
          peer-config (assoc (-> "peer-config.edn" resource slurp read-string) :onyx/tenancy-id onyx-id)
